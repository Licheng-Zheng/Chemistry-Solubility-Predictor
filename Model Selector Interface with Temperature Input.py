...
class ModelSelector:
    ...
    def setup_gui(self):
        ...
        self.root.geometry("600x450")  # Increased height to accommodate temperature input
        ...
        # Temperature input
        tk.Label(input_frame, text="Temperature (K):").grid(row=1, column=0, sticky=tk.W, pady=(10, 0))
        self.temp_var = tk.StringVar(value="298.15")  # Default to room temperature
        tk.Entry(input_frame, textvariable=self.temp_var, width=50).grid(row=1, column=1, padx=5, pady=(10, 0))
        
        # Predict button
        tk.Button(input_frame, text="Predict", command=self.make_prediction).grid(row=1, column=2, padx=5, pady=(10, 0))
    ...
    def prepare_input(self, input_data, temperature=None):
        ...
        if isinstance(input_data, str) and not input_data[0].isdigit():
            ...
            input_array = np.array(elements_list)
        ...
        # Add temperature to input if provided
        if temperature is not None:
            try:
                temp_value = float(temperature)
                # Append temperature to the input array
                input_array = np.append(input_array, temp_value)
            except ValueError:
    ...
    def make_prediction(self, input_data=None, temperature=None):
        ...
        try:
            ...
            if input_data is None and hasattr(self, 'input_var'):
                input_data = self.input_var.get()
            elif input_data is None:
                input_data = input("Enter chemical formula or space-separated numbers: ")
            
            # Get temperature from GUI or use provided temperature
            if temperature is None and hasattr(self, 'temp_var'):
                temperature = self.temp_var.get()
            elif temperature is None:
                temperature = input("Enter temperature (K): ")
                
            # Prepare input data with temperature
            input_array = self.prepare_input(input_data, temperature)
            ...
            result_message += f"Temperature: {temperature} K\n"
...
def predict_with_model(model_path, input_data, temperature=None):
    """Command-line function to make predictions with a model"""
    selector = ModelSelector()
    if selector.load_model(model_path):
        return selector.make_prediction(input_data, temperature)
    return None
